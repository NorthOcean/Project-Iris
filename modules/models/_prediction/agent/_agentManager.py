...
from ... import base
from ..._helpmethods import (calculate_ADE_FDE_numpy, dir_check,
...
class OnlineAgentManager(BaseAgentManager):
    ...
    --------------------
    An online version of `BaseAgentManager` that supports online prediction.

    FIXME
    """

    def __init__(self, args, agent_id, linear_predictor):
        """
        :param args: args to init this manager
        :param agent_id: agent's identifier
        :param linear_predictor: default linear predictor for this agent
        """
        super().__init__()
        self.obs_frames = args.obs_frames
        self.pred_frames = args.pred_frames
        self.total_frame = self.obs_frames + self.pred_frames
        self.obs_length = self.obs_frames
        self.wait_frames = args.wait_frames
    ...
    @property
    def traj(self):
        return np.array(self._traj[-self.obs_frames:])
    ...
    def traj(self, value):
        ...
        self._traj += value
        if len(self.traj) > self.obs_frames + self.wait_frames:
            self._traj = self._traj[-(self.obs_frames + self.wait_frames):]

    @BaseAgentManager.frame_list.setter
    def frame_list(self, frame_list):
        ...
        if len(self.traj) > self.obs_frames + self.wait_frames:
            self._frame_list = self._frame_list[-(
                self.obs_frames + self.wait_frames):]

    @property
    def pred_linear(self):
        if not type(self._traj_pred_linear) == type(None):
            return self._traj_pred_linear
        else:
            return self._traj_pred
...
class TrainAgentManager(BaseAgentManager):
    ...
    def calculate_loss(self, loss_function=calculate_ADE_FDE_numpy):
        self.loss = loss_function(self.pred, self.groundtruth)
        return self.loss
