...
- `--K_train`, type=`int`, changeable=`False`.
  The number of hidden behavior categories in `AlphaModel`, or the number of multiple generations when training in `BetaModel`.
  The default value is `10`.
- `--check`, type=`int`, changeable=`True`.
...
- `--loadb`, type=`str`, changeable=`True`.
  ...
  The default value is `'null'`.
- `--loadc`, type=`str`, changeable=`True`.
